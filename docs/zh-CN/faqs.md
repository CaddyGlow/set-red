...
如果登录成功但没有活动工作区，请重新加载 `/api/verify`。Access 身份会恢复有效选择；
已保存的成员关系被移除时会确定性地选择剩余工作区；完全没有成员关系时会创建一个所有者工作区。
管理界面不允许撤销最后一名已验证实例管理员。
...
仅当创建/编辑时**未设置** `unsafe` 才会自动检测。显式的 `true` 或 `false` 始终优先。DNS 检查失败时，Set 会放行链接。
