...
Set can use Cloudflare **Workers AI** to suggest short codes and social preview titles/descriptions. Optional — normal links work without it.
...
For a URL, Set tries to read the page and ask the model for structured output:

- `/api/link/ai` — short-code suggestion
- `/api/link/og-ai` — title and description; optional `locale` query for preferred language

If the model fails after the request starts, Set falls back to a simple URL-based suggestion. Always review before saving.
